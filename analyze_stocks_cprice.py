...
class ViewStocks:
    def __init__(self, conn):
        ...
        self.window.title("Stocks Price")
    ...
    def add_treeview(self, height):
        ...
        stock_tree.heading("#1", text="Customer Price")
        stock_tree.column("#2", anchor=tkinter.CENTER, stretch=tkinter.NO, width=200)
        stock_tree.heading("#2", text="Number of medicines")

        cur = self.conn.cursor()
        res = cur.execute("SELECT cust_unit_price, COUNT(sid) FROM stocks GROUP BY cust_unit_price " +
