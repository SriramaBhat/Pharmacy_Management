...
class ViewStocksName:
    ...
    def add_treeview(self, height):
        ...
        if self.emp_type == "admin":
            res = self.conn.cursor().execute("SELECT * FROM admin_stock_view ORDER BY mname").fetchall()
        else:
            res = self.conn.cursor().execute("SELECT * FROM emp_stock_view ORDER BY mname").fetchall()
