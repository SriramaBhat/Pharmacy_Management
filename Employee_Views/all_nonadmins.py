...
class ViewAllNonAdmin:
    def __init__(self, conn):
        self.window = tkinter.Tk()
        self.window.title("All Non Admins")
        self.window.geometry("1000x500")
        self.window.resizable(height=False, width=False)
        self.window.configure(background=DEFAULT_COLOR)
        self.conn = conn

        self.title = self.add_title("All Non Admins", 400, 50, ("bold", 20))
    ...
    def create_emp_view(self):
        ...
        cur.execute("CREATE VIEW employee_view AS " +
                    "SELECT ename, ephone, egender, eaddress, eemail FROM employee EXCEPT " +
